import 'package:digaxy/app/modules/helper/notification/widgets/helper_notification_card.dart';
...
class HelperNotificationView extends GetView<HelperNotificationController> {
  ...
  void _openDetails(Map<String, String> item) {
    ...
    final match = RegExp(r'\d+').firstMatch('$title $subtitle');
    final extractedJobId = match?.group(0);
    final jobId = parcelNumericId.isNotEmpty
        ? parcelNumericId
        : (extractedJobId ?? parcelId);

    Get.toNamed(
      '/helper/task/detail',
      arguments: {if (jobId.isNotEmpty) 'jobId': jobId},
    );
  }
